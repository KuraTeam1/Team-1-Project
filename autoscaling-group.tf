...
resource "aws_autoscaling_group" "appserver-autoscaling-group" {
  ...
  min_size             = "1"
  desired_capacity     = "3"
  vpc_zone_identifier  = ["${aws_subnet.private-subnet.0.id}", "${aws_subnet.private-subnet.1.id}", "${aws_subnet.private-subnet.2.id}"]
...
resource "aws_autoscaling_group" "dbserver-autoscaling-group" {
  ...
  min_size             = "1"
  desired_capacity     = "3"
  vpc_zone_identifier  = ["${aws_subnet.database-subnet.0.id}", "${aws_subnet.database-subnet.1.id}","${aws_subnet.database-subnet.2.id}"]
